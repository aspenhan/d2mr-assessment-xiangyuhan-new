...
I have converted the dataset to be in metric units for distances and weights and will perform some data visualisations and analysis.

I am interested in the relationship between fuel efficiency and horsepower, controlling for physical and design features such as weight, number of cylinders and carburetors.

I first compute some summary statistics of these variables of interest.
...
sum_stats <- tbl_summary(mtcars.final, by = transmission_manual,
  ...
  modify_header(label ~ "**Car Feature**") %>%
  modify_spanning_header(c("stat_1", "stat_2") ~ "**Manual transmission**") %>%
  modify_caption("**Summary statistics of car features**")

sum_stats

#When outputting to pdf/ latex, might need to convert table to kable format as below to ensure it renders correctly
#sum_stats %>% as_kable_extra(format = "latex", booktabs = TRUE, longtable = TRUE)

```

To delve deeper into the distributions of fuel efficiency and horsepower, I plot their density curves.
...
kmpg_density <- ggplot(data = mtcars.final, aes(x = kmpg)) + 
  ...
  labs(title = "Distribution of fuel efficiency", x = "Km per gallon", y = "Density") +
...
We see that both distributions are single-peaked, with the means 

Next, I am interested in the relationship between fuel efficiency and horsepower, both with and without controlling for weight.

```{r}
#| label: Kmpg-hp-scatterplot
...
Finally, I want to see how fuel efficiency and horsepower varies with the number of cylinders and carburetors.

```{r}
#| label: Horsepower histograms

# Histograms of horsepower grouped by number of cylinders and carburetors
...
hp__bycylcarb_hist <- ggplot(data = mtcars.final, aes(x = hp)) +
  ...
  labs(title = "Distribution of horsepower by number of carburetors and cylinders", x = "Horsepower", y = "Number of cars", fill = "Number of cylinders and carburetors") +
  guides(fill = "none") +
  theme(plot.title = element_text(family = "serif", size = (15), hjust = 0.5),
        axis.title = element_text(family = "serif", size = (13)),
        axis.text = element_text(family = "serif", size = (11)),
...
Moving on, I will perform some statistical analysis and inference on the data.
