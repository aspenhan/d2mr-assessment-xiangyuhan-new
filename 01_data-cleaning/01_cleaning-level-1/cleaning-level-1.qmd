...
Foremost, I convert the data to metric units because that is more standardised in the world of science.
...
# Now we get standard metric units yay; I will be working with this dataset in metric units

```

Now, I will perform some data visualisations.

I am first interested in the distribution of fuel efficiency and weight categories.

```{r}
#| label: Km per gallon categories barplot

kmpg_cat_bar <- ggplot(data = clean.mtcars.metric, aes(x = kmpg_category)) +
  geom_bar(fill = "lightblue") +
  labs(title = "Distribution of cars' fuel efficiency", x = "Km per gallon", y = "Number of cars") +
...
```{r}
#| label : Weight categories barplot

wt_cat_bar <- ggplot(data = clean.mtcars.metric, aes(x = wt_category, fill = wt_category)) +
  geom_bar(fill = "darkgreen") +
  labs(title = "Distribution of cars' weights", x = "Weight", y = "Number of cars") +
    theme(plot.title = element_text(family = "serif", size = (15), hjust = 0.5),
        axis.title = element_text(family = "serif", size = (13)),
        axis.text = element_text(family = "serif", size = (11)))
...
We see that most cars fall in the medium categories for both km per gallon and weight categories.

Next, I am interested in the relationship between fuel efficiency band horsepower, both with and without controlling for weight.

```{r}
#| label: Kmpg against hp scatterplots

hp_kmpg_scatter <- ggplot(data = clean.mtcars.metric, aes(x = hp, y = kmpg)) +
  geom_point(color = "red") +
  labs(title = "Relationship between fuel efficiency and horsepower", x = "Horsepower", y = "Km per gallon") + 
  theme(plot.title = element_text(family = "serif", size = (15), hjust = 0.5),
        axis.title = element_text(family = "serif", size = (13)),
        axis.text = element_text(family = "serif", size = (11)))
...
hp_kmpg_bywt_scatter <- ggplot(data = clean.mtcars.metric, aes(x = hp, y = kmpg)) +
  geom_point(aes(color = wt_category)) + facet_wrap(. ~ wt_category) +
  labs(title = "Fuel efficiency vs horsepower grouped by weight", x = "Horsepower", y = "Km per gallon", color = "Weight category") +
  theme(plot.title = element_text(family = "serif", size = (15), hjust = 0.5),
        axis.title = element_text(family = "serif", size = (13)),
        axis.text = element_text(family = "serif", size = (11)),
...
We see that in general, fuel efficiency is negatively correlated with horsepower, and after grouping by weight, we see a lower variability in the relationship for medium weight cars, with more of the variability coming from the heavy and light cars.

Finally, I want to see how horsepower varies with the number of cylinders and carburetors.

```{r}
#| label: Horsepower histograms by cylinders and carburetors

hp__bywtandcyl_hist <- ggplot(data = clean.mtcars.metric, aes(x = hp)) +
  geom_histogram(binwidth = 5, aes(fill = factor(interaction(cyl, carb)))) +
  facet_grid(carb ~ cyl, labeller = label_both) +
  labs(title = "Distribution of horsepower by number of carburetors and cylinders", x = "Horsepower", fill = "Number of cylinders and carburetors") +
  guides(fill = "none") +
  theme(plot.title = element_text(family = "serif", size = (15), hjust = 0.5),
        axis.title = element_text(family = "serif", size = (13)),
        axis.text = element_text(family = "serif", size = (11)),
...
We see that holding the number of carburetors constant, horsepower generally increases in the number of cylinders (reading horizontally across), and similarly, holding the number of cylinders constant, horsepower also generally increases in the number of carburetors (reading vertically down).

Moving on, I will perform some statistical inference on the data.
