...
# Create new variable mpg per number of cylinders
...
# Alternatively, if we did the above cleaning in a document, we can read in the exported data file by running the command below
# read.csv("clean-mtcars.csv")
# However, note we will need to reorder the factor levels and the rearrange the data to what we want

# Alternatively, if the cleaning was done in an R script, we can source the above R script using the command below
# source("name of R script file")
# Generally, this is preferred so it does not clutter the manuscript and it will preserve all the cleaning steps
...
sum_stats <- tbl_summary(mtcars.final, by = transmission_manual, #select variable by which observations will be grouped prior to computation
                         include = c("kmpg", "hp", "wt", "cyl", "carb", "transmission_manual"), #select variables for which summary statistics will be calculated
                         statistic = list(all_continuous() ~ "{mean} ({sd})",
                                          all_categorical() ~ "{n} ({p}%)"), #specify type of statistics to compute
                         label = list(kmpg ~ "Kilometres per gallon", hp ~ "Horsepower", wt ~ "Weight", cyl ~ "Cylinders", carb ~ "Carburetors") #rename the variables
                         ) %>%
  add_overall(last = TRUE) %>% #also compute for all (ungrouped) data
  modify_header(label ~ "**Characteristics**") %>% #rename the header of the variable column
  modify_spanning_header(c("stat_1", "stat_2") ~ "**Manual transmission**") #add header for the grouped by variable
...
kmpg_density <- ggplot(data = mtcars.final, aes(x = kmpg)) + #input dataset to be used, the variables for the x and y axis and associated aesthetics
  geom_density(color = "green", size = 1) + # specify the type of plot and aesthetic features for this plot type not associated with the axis variables 
  labs(x = "Km per gallon", y = "Density") + # customise the plot title, axis labels and values, legend title and text etc
  def_theme #customise the theme, which includes font type, size, colour, background colour for the different plot components
...
hp_kmpg_bywt_scatter <- ggplot(data = mtcars.final, aes(x = hp, y = kmpg)) +
  ...
  facet_wrap(. ~ wt_category) + #this segments the plots columnwise by the specified variable
...
kmpg__bycylcarb_hist <- ggplot(data = mtcars.final, aes(x = kmpg)) +
  ...
  facet_grid(carb ~ cyl, labeller = label_both) + #this segments rowwise by carb and columnwise by cyl
  labs(x = "Km per gallon", y = "Number of cars", fill = "Number of cylinders and carburetors") +
  guides(fill = "none") + #this removes the legend for the fill colours
  def_theme
